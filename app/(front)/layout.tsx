    ...
    return (
        <div className={"h-screen bg-black"}>
            <div className={"h-16 flex justify-between items-center px-2"}>
                <h2 className={"text-white text-xl font-extrabold"}>INTER-KNOT “{pathname}”</h2>
                <nav className={"border-2 border-white rounded-full"}>
                    <ul className={"tabs flex text-white font-bold text-xl"}>
                        {
                            navs.map((nav) => <li key={nav.href} className={"px-10 cursor-pointer py-2 relative"}>
                                <div
                                ...
                                <a href={nav.href}><span className={"z-20 relative text-black"}>推送</span></a>
                            </li>)
                        }
                    </ul>
                </nav>
